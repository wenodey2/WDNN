...
model_save_path = f"dataset2/results_speed/rnn/models/"
isExist = os.path.exists(model_save_path)
if not isExist:
    os.makedirs(model_save_path)
model.save(f'dataset2/results_speed/rnn/models/model_{turbine}_{hourahead}')
...
d = {'MAE': MAE, 'RMSE': RMSE}
df1 = pd.DataFrame(d)
df1.to_csv(f'dataset2/results_speed/rnn/metrics{turbine}_{hourahead}.csv', index=False)

d2 = {'actual_speed': y_test[12:], f'h={hourahead}': y_pred[12:]}
df2 = pd.DataFrame(d2)
df2.to_csv(f'dataset2/results_speed/rnn/windspeed_{turbine}_{hourahead}.csv', index=False)

toc = time.perf_counter()
time_elapsed = (toc - tic)/60
df3 = pd.DataFrame(np.array([time_elapsed]))
df3.to_csv(f'dataset2/results_speed/rnn/speed_time_{turbine}_{hourahead}.csv', index=False)
