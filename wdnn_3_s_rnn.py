...
turbine = 1    # run for all values from 1 to 100
...
# Train-Test Split
N_train = 43824  # train size
N_test = 12 + lag + 8736  # test size
split = 43824
...
cA_x_train = cA_x_train[:, :, np.newaxis]
...
cA_x_test = cA_x_test[:, :, np.newaxis]


# Deep Learning Model
def build_model():
    model = keras.Sequential([
        layers.SimpleRNN(lag, return_sequences=True, input_shape=[None, 1]),
...
model_save_path = f"dataset3/results_speed/rnn/models/"
isExist = os.path.exists(model_save_path)
if not isExist:
    os.makedirs(model_save_path)
model.save(f'dataset3/results_speed/rnn/models/model_{turbine}_{hourahead}')
...
d = {'MAE': MAE, 'RMSE': RMSE}
df1 = pd.DataFrame(d)
df1.to_csv(f'dataset3/results_speed/rnn/metrics{turbine}_{hourahead}.csv', index=False)

d2 = {'actual_speed': y_test[12:], f'h={hourahead}': y_pred[12:]}
df2 = pd.DataFrame(d2)
df2.to_csv(f'dataset3/results_speed/rnn/windspeed_{turbine}_{hourahead}.csv', index=False)

toc = time.perf_counter()
time_elapsed = (toc - tic)/60
df3 = pd.DataFrame(np.array([time_elapsed]))
df3.to_csv(f'dataset3/results_speed/rnn/speed_time_{turbine}_{hourahead}.csv', index=False)
