...
# Power Metrics
metrics_per_p_1 = metrics_persistence(data1, seq(4, 962, 6), 18293, 26471, 36, 12)
...
power_pred3 = array(NA, c(12, 2, 36))
for (t in 1:36) {
...
metrics_bin_p_1 = matrix(NA, nrow = 12, ncol = 2)
colnames(metrics_bin_p_1) = c('MAE', 'RMSE')
for (h in 1:12) {
  metrics_bin_p_1[h, 1] = mean(power_pred3[h, 1, ], na.rm = T)
  metrics_bin_p_1[h, 2] = mean(power_pred3[h, 2, ], na.rm = T)
}
...
# Power Metrics
metrics_df_p_1 = metrics_dftgp(1, 36, 12)
...
# Power Metrics
metrics_stan_p_1 = metrics_dftgp(1, 36, 12)
...
# Power Metrics
metrics_wdtgp_p_1 = metrics_wdtgp(1, 36, 12)


### WDNN (Power)
wdnn_power = read.csv(paste0('dataset1/results_power/ffnn/metrics_wdnn_p.csv'))
metrics_wdnn_p_1 = wdnn_power[1:12,]


# Table 9 MAE
hours = 1:12
tab9m = cbind(hours, metrics_per_p_1[,1], metrics_bin_p_1[,1], metrics_df_p_1[,1], 
              metrics_stan_p_1[,1], metrics_wdtgp_p_1[,1], metrics_wdnn_p_1[,1])
colnames(tab9m) = c("h (hours)", "Per", "Bin", "DF-TGP", "STAN-TGP", "WDNN(s)-TGP", "WDNN(p)")

# Table 9 RMSE
tab9r = cbind(hours, metrics_per_p_1[,2], metrics_bin_p_1[,2], metrics_df_p_1[,2], 
              metrics_stan_p_1[,2], metrics_wdtgp_p_1[,2], metrics_wdnn_p_1[,2])
colnames(tab9r) = c("h (hours)", "Per", "Bin", "DF-TGP", "STAN-TGP", "WDNN(s)-TGP", "WDNN(p)")


# Write to File
write.csv(tab9m,'tables/table9.csv', row.names = F)
